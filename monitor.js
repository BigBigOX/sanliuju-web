...
const ROUND_NAMES = {1:'信息采集',2:'当下匹配',3:'人性博弈'}

// ---- Data Collection ----
const alerts = []
function record() {}  // no-op
  ...
  try {
    const resp = await fetch(AI_BASE + '/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'deepseek-chat',
        messages: [
          { role: 'system', content: '你是三六局聚会游戏的后台裁判。简洁专业，每条一句话。' },
...
function handle(msg) {
  ...
  record({ type, from, data: data != null ? JSON.parse(JSON.stringify(data)) : null })
  ...
  switch (type) {
    ...
    case 'ROOM_STATE':
      ...
      joined.forEach(p => log_('  👤', `${p.avatar||'?'} ${p.nickname||p.id} ${p.isHost?'👑':''}`))
    ...
    case 'MATCH_RESULT':
      log_('📊', `AI 匹配结果: ${data.analysis?.slice(0,40)||''}`)
      break
    ...
    case 'ROUND_SETTLEMENT':
      log_('📋', `轮${data.round}结算: ${data.summary?.ai_comment?.slice(0,40)||''}`)
      break
    ...
    case 'GAME_ENDED':
      ...
      log_('  📊', `共${stats.questions.length}题 ${alerts.length}告警 耗时${Math.round(duration/1000)}s`)
      ...
      // AI final audit
      const summary = `游戏结束。${stats.questions.length}题，${alerts.length}告警。各轮题目：${JSON.stringify(stats.questions.map(q=>`轮${q.round}[${q.level}]${q.text.slice(0,20)}`))}。默契度前三：${top3.map(([k,v])=>k+':'+v).join(',')}。`
